...
class wm2graph:
    def __init__(self, project_folder=None):
        ...
        try:
            ...
            for file in file_list:
                if file.lower().endswith(".asm"):
                    ...
                    self.files["Load Control Points"] = os.path.join(
                        project_folder, file
                    )
                    ...
                    self.files["Structure Locations"] = os.path.join(
                        project_folder, file
                    )
            ...
            if len(self.files) == 0:
                raise ValueError(
                    'Folder path "{}" does not contain project files'.format(
                        project_folder
                    )
                )
        except:
        ...
        for table_name, table_path in self.files.items():
            ...
            try:
                ...
                logging.warning(
                    'File at ".{}" was not read correctly.'.format(table_path)
                )
    ...
    def iterate_nodes(self, Node1, Node2, Ppty):
        ...
        if Ppty not in self.nxGraph[Node1][Node2]:
            ...
            if inEdge:
                inEdgeNodes = inEdge.__iter__().__next__()
    ...
    def create_element_libraries(self):
        ...
        if "Equipment Data" in self.tables:
            ...
            for id, class_name in class_id_lookup.items():
                ...
                element_type_library[class_name] = element_type_data.loc[
                    element_type_data[1] == id
                ]
    ...
    def create_plot(self, updateCoordinates):
        ...
        if updateCoordinates:
            ...
            for edge in edges:
                node1, node2 = edge
                if (
                    "x" in self.nxGraph.nodes[node1]
                    and "x" in self.nxGraph.nodes[node2]
                ):
                ...
                for node in [node1, node2]:
                    if "loads" in self.nxGraph.nodes[node]:
                        for load_name, load_properties in self.nxGraph.nodes[node][
                            ...
                            if (
                                "x" in self.nxGraph.nodes[node]
                                and self.nxGraph.nodes[node]
                            ):
        ...
        # plot loads
        plot.triangle(x=load_x, y=load_y, legend_label="Loads", color="orange")
        # plot substation
    ...
    def create_generators(self):
        ...
        for r, generator in generators.iterrows():
            ...
            if from_node in self.nxGraph.nodes():
                ...
                self.nxGraph.nodes[from_node]["capacitors"][
                    self.fix_string(generator["Element Name"])
                ] = {
        ...
        for r, capacitor in capacitors.iterrows():
            ...
            if from_node in self.nxGraph.nodes():
                if "capacitors" not in self.nxGraph.nodes[from_node]:
                    self.nxGraph.nodes[from_node]["capacitors"] = {}
                self.nxGraph.nodes[from_node]["capacitors"][
                    self.fix_string(capacitor["Element Name"])
                ] = {
                    ...
                    "reg mode": capacitor_control_type[
                        int(capacitor["Switch Type Code"])
                    ],
                    "kvRated": capacitor["Voltage Rating"],
                    "kvar": [
                        capacitor["kvar, Phase A"],
    ...
    def create_loads(self):
        ...
        for r, load in service_locations.iterrows():
            ...
            if from_node in self.nxGraph.nodes():
                ...
                self.nxGraph.nodes[from_node]["loads"][
                    self.fix_string(load["Element Name"])
                ] = {
    ...
    def create_transformers(self):
        ...
        for r, xfmr in xfmrs.iterrows():
            ...
            fuse_dict = {
                ...
                "kv": [
                    xfmr["Rated Input Voltage (Src Side)"],
                ...
                "kva": [xfmr["Tran kVA A"], xfmr["Tran kVA B"], xfmr["Tran kVA C"]],
                "regulators": [
                    xfmr["Xfmr Cond Desc. Ph A"],
    ...
    def create_devices(self):
        ...
        for r, fuse in fuses.iterrows():
            ...
            fuse_dict = {
                ...
                "equipment": [
                    fuse["Description, Ph A"],
                    fuse["Description, Ph B"],
                    fuse["Description, Ph C"],
                ],
                "isClosed": [
                    fuse["Is Closed, Phase A"],
